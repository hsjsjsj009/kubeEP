...
func newClusterHandler(validatorInst *validator.Validate, generalClusterUC useCase.Cluster, db *gorm.DB, gcpClusterUC useCase.GCPCluster, gcpDatacenterUC useCase.GCPDatacenter, generalDatacenterUC useCase.Datacenter) ClusterHandler {
	return &clusterHandler{validatorInst: validatorInst, generalClusterUC: generalClusterUC, db: db, gcpClusterUC: gcpClusterUC, gcpDatacenterUC: gcpDatacenterUC, generalDatacenterUC: generalDatacenterUC}
}
...
func (ch *clusterHandler) GetAllHPA(c *fiber.Ctx) error {
	...
	clusterData, err := ch.generalClusterUC.GetClusterAndDatacenterDataByClusterID(ch.db, *requestData.ClusterID)
	if err != nil {
		return ch.errorResponse(c, err.Error())
	}
	...
	case constant.GCP:
		...
		googleCredential, err := ch.gcpDatacenterUC.GetGoogleCredentials(c.Context(), datacenterData)
		if err != nil {
			return ch.errorResponse(c, err.Error())
		}
		ch.gcpClusterUC.RegisterGoogleCredentials(datacenterName, googleCredential)
		kubernetesClient, err = ch.gcpClusterUC.GetKubernetesClusterClient(datacenterName, clusterData)
		if err != nil {
			return ch.errorResponse(c, err.Error())
		}
	default:
	...
	for _, hpa := range HPAs {
		responses = append(responses, response.SimpleHPA{
